...
def test_satisfies_protocol() -> None:
    assert isinstance(PEValue(), StrategyProtocol)


def test_metadata() -> None:
    s = PEValue()
    assert s.name == "pe_value"
...
def test_empty_input() -> None:
    ...
    assert PEValue().generate_signals(empty).empty


def test_single_asset_returns_zero() -> None:
    prices = _panel()
    single = prices[["A"]]
    weights = PEValue().generate_signals(single)
    assert (weights == 0.0).all().all()


def test_warmup_weights_are_zero() -> None:
    prices = _panel()
    weights = PEValue(lookback=756).generate_signals(prices)
...
def test_long_only_mode() -> None:
    prices = _panel()
    weights = PEValue(lookback=756, long_only=True).generate_signals(prices)
    assert (weights >= -1e-10).all().all()


def test_deterministic() -> None:
    prices = _panel()
    a = PEValue().generate_signals(prices)
    b = PEValue().generate_signals(prices)
