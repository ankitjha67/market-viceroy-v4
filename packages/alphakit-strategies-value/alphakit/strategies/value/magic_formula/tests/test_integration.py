"""Integration test for magic_formula."""
...
def test_magic_formula_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=MagicFormula(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "magic_formula"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_magic_formula_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=MagicFormula(), prices=prices)
    b = vectorbt_bridge.run(strategy=MagicFormula(), prices=prices)
