"""Integration test for piotroski_fscore_proxy."""
...
def test_piotroski_fscore_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=PiotroskiFScoreProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "piotroski_fscore_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_piotroski_fscore_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=PiotroskiFScoreProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=PiotroskiFScoreProxy(), prices=prices)
