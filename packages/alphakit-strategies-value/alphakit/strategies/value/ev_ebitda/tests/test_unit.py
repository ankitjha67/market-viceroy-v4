...
def test_satisfies_protocol() -> None:
    assert isinstance(EVEbitda(), StrategyProtocol)


def test_metadata() -> None:
    s = EVEbitda()
    assert s.name == "ev_ebitda"
...
def test_empty_input() -> None:
    ...
    assert EVEbitda().generate_signals(empty).empty


def test_single_asset_returns_zero() -> None:
    prices = _panel()
    single = prices[["A"]]
    weights = EVEbitda().generate_signals(single)
    assert (weights == 0.0).all().all()


def test_warmup_weights_are_zero() -> None:
    prices = _panel()
    weights = EVEbitda(lookback=756).generate_signals(prices)
...
def test_long_only_mode() -> None:
    prices = _panel()
    weights = EVEbitda(lookback=756, long_only=True).generate_signals(prices)
    assert (weights >= -1e-10).all().all()


def test_deterministic() -> None:
    prices = _panel()
    a = EVEbitda().generate_signals(prices)
    b = EVEbitda().generate_signals(prices)
