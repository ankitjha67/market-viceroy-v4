"""Integration test for country_cape_rotation."""
...
def test_country_cape_rotation_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=CountryCapeRotation(lookback=252), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "country_cape_rotation"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_country_cape_rotation_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=CountryCapeRotation(lookback=252), prices=prices)
    b = vectorbt_bridge.run(strategy=CountryCapeRotation(lookback=252), prices=prices)
