"""Integration test for altman_zscore_proxy."""
...
def test_altman_zscore_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=AltmanZScoreProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "altman_zscore_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_altman_zscore_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=AltmanZScoreProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=AltmanZScoreProxy(), prices=prices)
