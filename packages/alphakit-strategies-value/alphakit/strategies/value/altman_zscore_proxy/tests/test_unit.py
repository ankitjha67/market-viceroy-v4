"""Unit tests for altman_zscore_proxy."""
...
def test_satisfies_protocol() -> None:
    assert isinstance(AltmanZScoreProxy(), StrategyProtocol)


def test_metadata() -> None:
    s = AltmanZScoreProxy()
    assert s.name == "altman_zscore_proxy"
    assert s.family == "value"


def test_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["A"], dtype=float)
    assert AltmanZScoreProxy().generate_signals(empty).empty
...
def test_single_asset_returns_zero() -> None:
    ...
    weights = AltmanZScoreProxy().generate_signals(prices)
...
def test_long_only_mode() -> None:
    prices = _panel()
    s = AltmanZScoreProxy(long_only=True)
...
def test_deterministic() -> None:
    prices = _panel()
    a = AltmanZScoreProxy().generate_signals(prices)
    b = AltmanZScoreProxy().generate_signals(prices)
