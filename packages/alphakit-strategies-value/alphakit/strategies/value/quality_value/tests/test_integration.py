"""Integration test for quality_value."""
...
def test_quality_value_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=QualityValue(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "quality_value"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_quality_value_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=QualityValue(), prices=prices)
    b = vectorbt_bridge.run(strategy=QualityValue(), prices=prices)
