...
def test_satisfies_protocol() -> None:
    assert isinstance(QualityValue(), StrategyProtocol)


def test_metadata() -> None:
    s = QualityValue()
    assert s.name == "quality_value"
    assert s.family == "value"


def test_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["A"], dtype=float)
    assert QualityValue().generate_signals(empty).empty
...
def test_single_asset_returns_zero() -> None:
    ...
    weights = QualityValue().generate_signals(prices)
...
def test_deterministic() -> None:
    prices = _panel()
    a = QualityValue().generate_signals(prices)
    b = QualityValue().generate_signals(prices)
