"""Integration test for shareholder_yield."""
...
def test_shareholder_yield_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=ShareholderYield(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "shareholder_yield"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_shareholder_yield_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=ShareholderYield(), prices=prices)
    b = vectorbt_bridge.run(strategy=ShareholderYield(), prices=prices)
