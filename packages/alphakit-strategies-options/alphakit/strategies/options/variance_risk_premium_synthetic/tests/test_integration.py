"""Integration test for variance_risk_premium_synthetic."""
...
def test_full_short_atm_straddle_runs_end_to_end() -> None:
    underlying = _deterministic_underlying()
    chain_feed = SyntheticOptionsFeed(underlying_feed=_FakeUnderlying(underlying))
    strategy = VarianceRiskPremiumSynthetic(chain_feed=chain_feed)
    ...
    assert result.meta["strategy"] == "variance_risk_premium_synthetic"
