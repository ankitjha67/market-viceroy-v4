"""Unit tests for commodity_tsmom signal generation.

Mirror of bond_tsmom_12_1's unit-test suite, adapted for the multi-
asset commodity panel: single-asset behaviours are tested per column
on synthetic exponentially-trending paths, and multi-asset behaviour
is tested on a mixed panel where some commodities trend up and
others trend down.
"""
...
def _mixed_panel(years: float = 3) -> pd.DataFrame:
    """Half the commodities trend up, half trend down. Used to test
    multi-asset cross-sectional behaviour (long top half, short bottom).
    """
    index = _daily_index(years)
    n = len(index)
    drifts = {
        ...
        "GC=F": +0.0008,  # uptrend
        "HG=F": +0.0006,  # uptrend
        "ZC=F": +0.0004,  # mild uptrend
        "NG=F": -0.0010,  # downtrend
        "SI=F": -0.0008,  # downtrend
        "ZS=F": -0.0006,  # downtrend
        "ZW=F": -0.0004,  # mild downtrend
    }
    return pd.DataFrame(
...
def test_satisfies_strategy_protocol() -> None:
    assert isinstance(CommodityTSMOM12m1m(), StrategyProtocol)


def test_metadata_is_paper_cited() -> None:
    s = CommodityTSMOM12m1m()
    assert s.name == "commodity_tsmom"
...
def test_constructor_rejects_bad_args(kwargs: dict[str, object], match: str) -> None:
    with pytest.raises(ValueError, match=match):
        CommodityTSMOM12m1m(**kwargs)  # type: ignore[arg-type]
...
def test_returns_empty_frame_on_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["CL=F"], dtype=float)
    weights = CommodityTSMOM12m1m().generate_signals(empty)
    assert weights.empty
    assert list(weights.columns) == ["CL=F"]


def test_output_is_aligned_to_input() -> None:
    prices = _trending_panel(["CL=F", "GC=F"], years=3, daily_drift=0.0004)
    weights = CommodityTSMOM12m1m().generate_signals(prices)
    assert weights.index.equals(prices.index)
    assert list(weights.columns) == ["CL=F", "GC=F"]
    assert weights.to_numpy().dtype == np.float64


def test_rejects_non_dataframe_input() -> None:
    with pytest.raises(TypeError, match="DataFrame"):
        CommodityTSMOM12m1m().generate_signals([1, 2, 3])  # type: ignore[arg-type]


def test_rejects_non_datetime_index() -> None:
    prices = pd.DataFrame({"CL=F": [100.0, 101.0]}, index=[0, 1])
    with pytest.raises(TypeError, match="DatetimeIndex"):
        CommodityTSMOM12m1m().generate_signals(prices)


def test_rejects_non_positive_prices() -> None:
    prices = _trending_panel(["CL=F"], years=2, daily_drift=0.0001)
...
def test_warmup_weights_are_zero() -> None:
    ...
    prices = _trending_panel(["CL=F", "GC=F"], years=3, daily_drift=0.0005)
    weights = CommodityTSMOM12m1m().generate_signals(prices)
...
def test_uptrend_emits_long_weights() -> None:
    """Monotone uptrend on each commodity → strictly positive weights."""
    prices = _trending_panel(["CL=F", "GC=F"], years=3, daily_drift=0.001)
    weights = CommodityTSMOM12m1m().generate_signals(prices)
...
def test_constant_prices_emit_zero_weights() -> None:
    ...
    prices = pd.DataFrame(
        {"CL=F": np.full(len(index), 100.0), "GC=F": np.full(len(index), 100.0)},
        index=index,
    )
    weights = CommodityTSMOM12m1m().generate_signals(prices)
...
def test_weights_respect_leverage_cap() -> None:
    ...
    prices = _trending_panel(["CL=F"], years=3, daily_drift=0.0001)
    strategy = CommodityTSMOM12m1m(max_leverage_per_asset=1.5)
    weights = strategy.generate_signals(prices)
    assert weights["CL=F"].abs().max() <= 1.5 + 1e-9


def test_weights_change_only_at_month_ends() -> None:
    """Monthly rebalance: weights are piecewise-constant within a month."""
    prices = _trending_panel(["CL=F"], years=3, daily_drift=0.0005)
    weights = CommodityTSMOM12m1m().generate_signals(prices)
    idx = weights.index
    assert isinstance(idx, pd.DatetimeIndex)
    by_month = weights["CL=F"].groupby(idx.to_period("M")).nunique()
    assert (by_month <= 2).all(), "weights must be piecewise-constant within months"


def test_deterministic_output() -> None:
    prices = _trending_panel(["CL=F", "GC=F"], years=3, daily_drift=0.0003)
    w1 = CommodityTSMOM12m1m().generate_signals(prices)
    w2 = CommodityTSMOM12m1m().generate_signals(prices)
...
def test_mixed_panel_produces_long_and_short_legs() -> None:
    """On a mixed panel where 4 commodities trend up and 4 trend down,
    the strategy must produce 4 long and 4 short legs after warm-up.
    """
...
def test_per_asset_weights_independent_when_no_cross_signal() -> None:
    ...
    a panel with other independent commodities.
    """
    cl_alone = CommodityTSMOM12m1m().generate_signals(
        _trending_panel(["CL=F"], years=3, daily_drift=0.0008)
    )

    panel = _trending_panel(["CL=F", "GC=F"], years=3, daily_drift=0.0008)
    panel_weights = CommodityTSMOM12m1m().generate_signals(panel)

    # CL=F column should match between the single-asset and panel runs.
    pd.testing.assert_series_equal(cl_alone["CL=F"], panel_weights["CL=F"], check_names=False)
