"""Unit tests for metals_momentum signal generation.

Mirror of commodity_tsmom's unit-test suite, adapted for the
metals-only universe: single-asset behaviours are tested per column
on synthetic exponentially-trending paths, and multi-asset behaviour
is tested on a mixed metals panel where some metals trend up and
others trend down.
"""
...
def _mixed_metals_panel(years: float = 3) -> pd.DataFrame:
    """Half the metals trend up (monetary), half trend down (industrial).

    Used to test multi-asset behaviour: each metal independently
    receives a long or short weight per its own trend.
    """
    index = _daily_index(years)
    n = len(index)
    drifts = {
        "GC=F": +0.0010,  # gold uptrend
        "SI=F": +0.0008,  # silver uptrend
        "HG=F": -0.0010,  # copper downtrend
        "PL=F": -0.0008,  # platinum downtrend
    }
    return pd.DataFrame(
...
def test_satisfies_strategy_protocol() -> None:
    assert isinstance(MetalsMomentum(), StrategyProtocol)


def test_metadata_is_paper_cited() -> None:
    s = MetalsMomentum()
    assert s.name == "metals_momentum"
    assert s.family == "commodity"
    assert s.paper_doi == "10.1111/jofi.12021"  # Asness/Moskowitz/Pedersen 2013 §V
...
def test_constructor_rejects_bad_args(kwargs: dict[str, object], match: str) -> None:
    with pytest.raises(ValueError, match=match):
        MetalsMomentum(**kwargs)  # type: ignore[arg-type]
...
def test_returns_empty_frame_on_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["GC=F"], dtype=float)
    weights = MetalsMomentum().generate_signals(empty)
    assert weights.empty
    assert list(weights.columns) == ["GC=F"]


def test_output_is_aligned_to_input() -> None:
    prices = _trending_panel(["GC=F", "SI=F"], years=3, daily_drift=0.0004)
    weights = MetalsMomentum().generate_signals(prices)
    assert weights.index.equals(prices.index)
    assert list(weights.columns) == ["GC=F", "SI=F"]
    assert weights.to_numpy().dtype == np.float64


def test_rejects_non_dataframe_input() -> None:
    with pytest.raises(TypeError, match="DataFrame"):
        MetalsMomentum().generate_signals([1, 2, 3])  # type: ignore[arg-type]


def test_rejects_non_datetime_index() -> None:
    prices = pd.DataFrame({"GC=F": [100.0, 101.0]}, index=[0, 1])
    with pytest.raises(TypeError, match="DatetimeIndex"):
        MetalsMomentum().generate_signals(prices)


def test_rejects_non_positive_prices() -> None:
    prices = _trending_panel(["GC=F"], years=2, daily_drift=0.0001)
...
def test_warmup_weights_are_zero() -> None:
    ...
    prices = _trending_panel(["GC=F", "SI=F"], years=3, daily_drift=0.0005)
    weights = MetalsMomentum().generate_signals(prices)
...
def test_uptrend_emits_long_weights() -> None:
    """Monotone uptrend on each metal → strictly positive weights."""
    prices = _trending_panel(["GC=F", "SI=F"], years=3, daily_drift=0.001)
    weights = MetalsMomentum().generate_signals(prices)
...
def test_constant_prices_emit_zero_weights() -> None:
    ...
    prices = pd.DataFrame(
        {"GC=F": np.full(len(index), 100.0), "SI=F": np.full(len(index), 100.0)},
        index=index,
    )
    weights = MetalsMomentum().generate_signals(prices)
...
def test_weights_respect_leverage_cap() -> None:
    ...
    prices = _trending_panel(["GC=F"], years=3, daily_drift=0.0001)
    strategy = MetalsMomentum(max_leverage_per_asset=1.5)
    weights = strategy.generate_signals(prices)
    assert weights["GC=F"].abs().max() <= 1.5 + 1e-9


def test_weights_change_only_at_month_ends() -> None:
    """Monthly rebalance: weights are piecewise-constant within a month."""
    prices = _trending_panel(["GC=F"], years=3, daily_drift=0.0005)
    weights = MetalsMomentum().generate_signals(prices)
    idx = weights.index
    assert isinstance(idx, pd.DatetimeIndex)
    by_month = weights["GC=F"].groupby(idx.to_period("M")).nunique()
    assert (by_month <= 2).all(), "weights must be piecewise-constant within months"


def test_deterministic_output() -> None:
    prices = _trending_panel(["GC=F", "SI=F"], years=3, daily_drift=0.0003)
    w1 = MetalsMomentum().generate_signals(prices)
    w2 = MetalsMomentum().generate_signals(prices)
...
# ---------------------------------------------------------------------------
def test_mixed_panel_produces_long_and_short_legs() -> None:
    """On a mixed metals panel where 2 trend up and 2 trend down, the
    strategy must produce 2 long and 2 short legs after warm-up.
    """
    prices = _mixed_metals_panel(years=3)
    weights = MetalsMomentum().generate_signals(prices)
    final = weights.iloc[-1]

    long_legs = (final > 0).sum()
    short_legs = (final < 0).sum()
    flat_legs = (final == 0).sum()

    assert long_legs == 2, f"expected 2 long legs, got {long_legs}"
    assert short_legs == 2, f"expected 2 short legs, got {short_legs}"
...
def test_per_asset_weights_independent_when_no_cross_signal() -> None:
    ...
    a panel with other independent metals.
    """
    gc_alone = MetalsMomentum().generate_signals(
        _trending_panel(["GC=F"], years=3, daily_drift=0.0008)
    )

    panel = _trending_panel(["GC=F", "SI=F"], years=3, daily_drift=0.0008)
    panel_weights = MetalsMomentum().generate_signals(panel)

    pd.testing.assert_series_equal(gc_alone["GC=F"], panel_weights["GC=F"], check_names=False)
