"""Integration test for iron_condor_systematic_proxy."""
...
def test_iron_condor_systematic_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=IronCondorSystematicProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "iron_condor_systematic_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_iron_condor_systematic_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=IronCondorSystematicProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=IronCondorSystematicProxy(), prices=prices)
