"""Integration test for vol_targeting."""
...
def test_vol_targeting_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=VolTargeting(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "vol_targeting"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_vol_targeting_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=VolTargeting(), prices=prices)
    b = vectorbt_bridge.run(strategy=VolTargeting(), prices=prices)
