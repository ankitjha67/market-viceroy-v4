"""Integration test for vrp_harvest."""
...
def test_vrp_harvest_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=VRPHarvest(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "vrp_harvest"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_vrp_harvest_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=VRPHarvest(), prices=prices)
    b = vectorbt_bridge.run(strategy=VRPHarvest(), prices=prices)
