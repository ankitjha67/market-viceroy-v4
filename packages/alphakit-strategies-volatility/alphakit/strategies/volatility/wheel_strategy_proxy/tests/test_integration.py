"""Integration test for wheel_strategy_proxy."""
...
def test_wheel_strategy_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=WheelStrategyProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "wheel_strategy_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_wheel_strategy_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=WheelStrategyProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=WheelStrategyProxy(), prices=prices)
