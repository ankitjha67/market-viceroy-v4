"""Unit tests for short_strangle_proxy."""
...
def test_satisfies_protocol() -> None:
    assert isinstance(ShortStrangleProxy(), StrategyProtocol)


def test_metadata() -> None:
    s = ShortStrangleProxy()
    assert s.name == "short_strangle_proxy"
    assert s.family == "volatility"


def test_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["SPY"], dtype=float)
    assert ShortStrangleProxy().generate_signals(empty).empty


def test_warmup_weights_are_zero() -> None:
    prices = _panel()
    weights = ShortStrangleProxy().generate_signals(prices)
...
def test_long_only_mode() -> None:
    prices = _panel()
    s = ShortStrangleProxy(long_only=True)
...
def test_deterministic() -> None:
    prices = _panel()
    a = ShortStrangleProxy().generate_signals(prices)
    b = ShortStrangleProxy().generate_signals(prices)
    pd.testing.assert_frame_equal(a, b)


def test_rejects_non_dataframe() -> None:
    with pytest.raises(TypeError, match="DataFrame"):
        ShortStrangleProxy().generate_signals("not a df")  # type: ignore[arg-type]


def test_rejects_non_datetime_index() -> None:
    prices = pd.DataFrame({"A": [100.0, 101.0, 102.0]})
    with pytest.raises(TypeError, match="DatetimeIndex"):
        ShortStrangleProxy().generate_signals(prices)
