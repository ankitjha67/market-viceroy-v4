"""Integration test for short_strangle_proxy."""
...
def test_short_strangle_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=ShortStrangleProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "short_strangle_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_short_strangle_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=ShortStrangleProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=ShortStrangleProxy(), prices=prices)
