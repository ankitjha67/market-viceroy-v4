"""Integration test for covered_call_proxy."""
...
def test_covered_call_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=CoveredCallProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "covered_call_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_covered_call_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=CoveredCallProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=CoveredCallProxy(), prices=prices)
