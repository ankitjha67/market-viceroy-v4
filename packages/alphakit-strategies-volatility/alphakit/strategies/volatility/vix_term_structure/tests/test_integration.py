"""Integration test for vix_term_structure."""
...
def test_vix_term_structure_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=VIXTermStructure(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "vix_term_structure"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_vix_term_structure_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=VIXTermStructure(), prices=prices)
    b = vectorbt_bridge.run(strategy=VIXTermStructure(), prices=prices)
