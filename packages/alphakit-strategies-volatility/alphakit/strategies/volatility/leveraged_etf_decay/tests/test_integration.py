"""Integration test for leveraged_etf_decay."""
...
def test_leveraged_etf_decay_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=LeveragedETFDecay(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "leveraged_etf_decay"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_leveraged_etf_decay_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=LeveragedETFDecay(), prices=prices)
    b = vectorbt_bridge.run(strategy=LeveragedETFDecay(), prices=prices)
