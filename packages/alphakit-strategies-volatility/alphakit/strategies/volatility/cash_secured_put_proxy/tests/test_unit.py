"""Unit tests for cash_secured_put_proxy."""
...
def test_satisfies_protocol() -> None:
    assert isinstance(CashSecuredPutProxy(), StrategyProtocol)


def test_metadata() -> None:
    s = CashSecuredPutProxy()
    assert s.name == "cash_secured_put_proxy"
    assert s.family == "volatility"


def test_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["SPY"], dtype=float)
    assert CashSecuredPutProxy().generate_signals(empty).empty


def test_warmup_weights_are_zero() -> None:
    prices = _panel()
    weights = CashSecuredPutProxy().generate_signals(prices)
...
def test_long_only_mode() -> None:
    prices = _panel()
    s = CashSecuredPutProxy(long_only=True)
...
def test_deterministic() -> None:
    prices = _panel()
    a = CashSecuredPutProxy().generate_signals(prices)
    b = CashSecuredPutProxy().generate_signals(prices)
    pd.testing.assert_frame_equal(a, b)


def test_rejects_non_dataframe() -> None:
    with pytest.raises(TypeError, match="DataFrame"):
        CashSecuredPutProxy().generate_signals("not a df")  # type: ignore[arg-type]


def test_rejects_non_datetime_index() -> None:
    prices = pd.DataFrame({"A": [100.0, 101.0, 102.0]})
    with pytest.raises(TypeError, match="DatetimeIndex"):
        CashSecuredPutProxy().generate_signals(prices)
