"""Integration test for cash_secured_put_proxy."""
...
def test_cash_secured_put_proxy_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=CashSecuredPutProxy(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "cash_secured_put_proxy"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_cash_secured_put_proxy_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=CashSecuredPutProxy(), prices=prices)
    b = vectorbt_bridge.run(strategy=CashSecuredPutProxy(), prices=prices)
