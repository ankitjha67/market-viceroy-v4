"""Integration test for rsi_reversion_2."""
...
def test_rsi_reversion_2_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=RSIReversion2(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "rsi_reversion_2"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_rsi_reversion_2_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=RSIReversion2(), prices=prices)
    b = vectorbt_bridge.run(strategy=RSIReversion2(), prices=prices)
