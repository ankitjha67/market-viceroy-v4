"""Integration test for overnight_intraday."""
...
def test_overnight_intraday_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=OvernightIntraday(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "overnight_intraday"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_overnight_intraday_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=OvernightIntraday(), prices=prices)
    b = vectorbt_bridge.run(strategy=OvernightIntraday(), prices=prices)
