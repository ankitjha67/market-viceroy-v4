...
def test_gap_fill_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=GapFill(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "gap_fill"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_gap_fill_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=GapFill(), prices=prices)
    b = vectorbt_bridge.run(strategy=GapFill(), prices=prices)
