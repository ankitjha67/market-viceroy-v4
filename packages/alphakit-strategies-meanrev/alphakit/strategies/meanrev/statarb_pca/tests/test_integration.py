"""Integration test for statarb_pca."""
...
def test_statarb_pca_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=StatArbPCA(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "statarb_pca"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_statarb_pca_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=StatArbPCA(), prices=prices)
    b = vectorbt_bridge.run(strategy=StatArbPCA(), prices=prices)
