"""Integration test for long_term_reversal."""
...
def test_long_term_reversal_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=LongTermReversal(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "long_term_reversal"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_long_term_reversal_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=LongTermReversal(), prices=prices)
    b = vectorbt_bridge.run(strategy=LongTermReversal(), prices=prices)
