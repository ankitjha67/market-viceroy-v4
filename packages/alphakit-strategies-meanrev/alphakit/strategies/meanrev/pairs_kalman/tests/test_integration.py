"""Integration test for pairs_kalman."""
...
def test_pairs_kalman_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=PairsKalman(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "pairs_kalman"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_pairs_kalman_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=PairsKalman(), prices=prices)
    b = vectorbt_bridge.run(strategy=PairsKalman(), prices=prices)
