"""Integration test for pairs_johansen."""
...
def test_pairs_johansen_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=PairsJohansen(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "pairs_johansen"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_pairs_johansen_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=PairsJohansen(), prices=prices)
    b = vectorbt_bridge.run(strategy=PairsJohansen(), prices=prices)
