"""Integration test for short_term_reversal_1m."""
...
def test_short_term_reversal_1m_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=ShortTermReversal1M(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "short_term_reversal_1m"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_short_term_reversal_1m_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=ShortTermReversal1M(), prices=prices)
    b = vectorbt_bridge.run(strategy=ShortTermReversal1M(), prices=prices)
