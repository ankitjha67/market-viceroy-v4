"""Integration test for ou_process_trade."""
...
def test_ou_process_trade_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=OUProcessTrade(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "ou_process_trade"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_ou_process_trade_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=OUProcessTrade(), prices=prices)
    b = vectorbt_bridge.run(strategy=OUProcessTrade(), prices=prices)
