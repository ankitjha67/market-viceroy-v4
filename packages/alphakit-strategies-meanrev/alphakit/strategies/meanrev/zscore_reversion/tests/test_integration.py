"""Integration test for zscore_reversion."""
...
def test_zscore_reversion_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=ZScoreReversion(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "zscore_reversion"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_zscore_reversion_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=ZScoreReversion(), prices=prices)
    b = vectorbt_bridge.run(strategy=ZScoreReversion(), prices=prices)
