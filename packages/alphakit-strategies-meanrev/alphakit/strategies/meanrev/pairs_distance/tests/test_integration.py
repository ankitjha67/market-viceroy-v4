"""Integration test for pairs_distance."""
...
def test_pairs_distance_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=PairsDistance(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "pairs_distance"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_pairs_distance_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=PairsDistance(), prices=prices)
    b = vectorbt_bridge.run(strategy=PairsDistance(), prices=prices)
