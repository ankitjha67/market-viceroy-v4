"""Integration test for pairs_engle_granger."""
...
def test_pairs_engle_granger_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=PairsEngleGranger(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "pairs_engle_granger"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_pairs_engle_granger_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=PairsEngleGranger(), prices=prices)
    b = vectorbt_bridge.run(strategy=PairsEngleGranger(), prices=prices)
