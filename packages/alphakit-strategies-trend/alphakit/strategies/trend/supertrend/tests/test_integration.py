"""Integration test for supertrend."""
...
def test_supertrend_runs() -> None:
    result = vectorbt_bridge.run(strategy=Supertrend(), prices=_panel())
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "supertrend"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_supertrend_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=Supertrend(), prices=prices)
    b = vectorbt_bridge.run(strategy=Supertrend(), prices=prices)
