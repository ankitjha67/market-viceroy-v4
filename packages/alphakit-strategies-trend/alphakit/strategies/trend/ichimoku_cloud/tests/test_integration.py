"""Integration test for ichimoku_cloud."""
...
def test_ichimoku_cloud_runs() -> None:
    result = vectorbt_bridge.run(strategy=IchimokuCloud(), prices=_panel())
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "ichimoku_cloud"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_ichimoku_cloud_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=IchimokuCloud(), prices=prices)
    b = vectorbt_bridge.run(strategy=IchimokuCloud(), prices=prices)
