"""Integration test for donchian_breakout_20."""
...
def test_donchian_breakout_20_runs() -> None:
    result = vectorbt_bridge.run(strategy=DonchianBreakout20(), prices=_panel())
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "donchian_breakout_20"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_donchian_breakout_20_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=DonchianBreakout20(), prices=prices)
    b = vectorbt_bridge.run(strategy=DonchianBreakout20(), prices=prices)
