"""Integration test for sma_cross_50_200."""
...
def test_sma_cross_50_200_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=SMACross50200(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "sma_cross_50_200"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_sma_cross_50_200_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=SMACross50200(), prices=prices)
    b = vectorbt_bridge.run(strategy=SMACross50200(), prices=prices)
