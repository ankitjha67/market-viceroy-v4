"""Integration test for turtle_full."""
...
def test_turtle_full_runs() -> None:
    result = vectorbt_bridge.run(strategy=TurtleFull(), prices=_panel())
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "turtle_full"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_turtle_full_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=TurtleFull(), prices=prices)
    b = vectorbt_bridge.run(strategy=TurtleFull(), prices=prices)
