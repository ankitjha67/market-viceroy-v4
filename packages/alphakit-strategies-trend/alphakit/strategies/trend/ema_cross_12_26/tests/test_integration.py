"""Integration test for ema_cross_12_26."""
...
def test_ema_cross_12_26_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=EMACross1226(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "ema_cross_12_26"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_ema_cross_12_26_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=EMACross1226(), prices=prices)
    b = vectorbt_bridge.run(strategy=EMACross1226(), prices=prices)
