"""Integration test for vol_carry_vrp."""
...
def _panel(seed: int = 42, years: float = 3) -> pd.DataFrame:
    ...
    cfgs = {"SPY": (0.0003, 0.012), "EFA": (0.0001, 0.010), "AGG": (0.00005, 0.004)}
    return pd.DataFrame(
...
def test_vol_carry_vrp_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=VolCarryVRP(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "vol_carry_vrp"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_vol_carry_vrp_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=VolCarryVRP(), prices=prices)
    b = vectorbt_bridge.run(strategy=VolCarryVRP(), prices=prices)
