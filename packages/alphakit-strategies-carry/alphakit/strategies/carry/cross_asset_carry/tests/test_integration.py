"""Integration test for cross_asset_carry."""
...
def test_cross_asset_carry_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=CrossAssetCarry(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "cross_asset_carry"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_cross_asset_carry_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=CrossAssetCarry(), prices=prices)
    b = vectorbt_bridge.run(strategy=CrossAssetCarry(), prices=prices)
