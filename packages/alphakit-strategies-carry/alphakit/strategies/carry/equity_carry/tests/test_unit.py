...
def test_satisfies_protocol() -> None:
    assert isinstance(EquityCarry(), StrategyProtocol)


def test_metadata() -> None:
    s = EquityCarry()
    assert s.name == "equity_carry"
...
def test_rejects_bad_args(kwargs: dict[str, object], match: str) -> None:
    with pytest.raises(ValueError, match=match):
        EquityCarry(**kwargs)  # type: ignore[arg-type]


def test_empty_input() -> None:
    empty = pd.DataFrame(index=pd.DatetimeIndex([]), columns=["SPY"], dtype=float)
    assert EquityCarry().generate_signals(empty).empty
...
def test_single_asset_returns_zero() -> None:
    ...
    weights = EquityCarry().generate_signals(prices)
    assert (weights == 0.0).all().all()


def test_warmup_weights_are_zero() -> None:
    prices = _panel()
    weights = EquityCarry(lookback=252).generate_signals(prices)
...
def test_long_only_mode() -> None:
    prices = _panel()
    weights = EquityCarry(long_only=True).generate_signals(prices)
    assert (weights >= -1e-10).all().all()


def test_deterministic() -> None:
    prices = _panel()
    a = EquityCarry().generate_signals(prices)
    b = EquityCarry().generate_signals(prices)
