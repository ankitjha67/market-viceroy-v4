"""Integration test for equity_carry."""
...
def test_equity_carry_runs() -> None:
    prices = _panel()
    result = vectorbt_bridge.run(strategy=EquityCarry(), prices=prices)
    assert isinstance(result, BacktestResult)
    assert result.meta["strategy"] == "equity_carry"
    for key in ("sharpe", "sortino", "calmar", "max_drawdown"):
        assert np.isfinite(result.metrics[key])


@pytest.mark.integration
def test_equity_carry_deterministic() -> None:
    prices = _panel(seed=1)
    a = vectorbt_bridge.run(strategy=EquityCarry(), prices=prices)
    b = vectorbt_bridge.run(strategy=EquityCarry(), prices=prices)
