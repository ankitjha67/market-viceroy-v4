"""Integration tests for vigilant_asset_allocation_5.

Mocks ``YFinanceAdapter.fetch`` to return a deterministic synthetic
5-ETF panel, runs the strategy through the vectorbt bridge, and
...
def _synthetic_vaa_panel(seed: int = 42, years: float = 5) -> pd.DataFrame:
    """Deterministic 5-ETF panel with realistic per-asset drift and vol."""
...
def test_backtest_runs_end_to_end_with_mocked_yfinance() -> None:
    panel = _synthetic_vaa_panel(seed=42, years=5)
    ...
    assert result.meta["strategy"] == "vigilant_asset_allocation_5"
    assert result.meta["paper_doi"] == "10.2139/ssrn.3002624"
...
def test_backtest_is_deterministic_with_mocked_yfinance() -> None:
    panel = _synthetic_vaa_panel(seed=123, years=4)
...
def test_backtest_rejects_empty_prices() -> None:
    ...
    with pytest.raises(ValueError, match="empty"):
        vectorbt_bridge.run(strategy=VigilantAssetAllocation5(), prices=empty)
